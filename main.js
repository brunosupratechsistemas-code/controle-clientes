/* =========================
   MAIN CONTROLLER
========================= */
window.contatosTempNovoCliente = [];
window.contatosExtrasTemp = [];
...
window.confirmAddClient = async function () {
  ...
  // 🔒 TRAVA DUPLO CLIQUE
  if (salvandoCliente) {
    console.warn("⚠️ Já está salvando, ignorando clique...");
    return;
  }
  ...
  try {
    ...
    // 🔹 CONTATOS EXTRAS
    if (window.contatosExtrasTemp?.length > 0) {

      for (const c of window.contatosExtrasTemp) {
        await addDoc(collection(db, "clientes", docRef.id, "contatos"), {
          nome: c.nome,
          telefone: c.telefone,
          createdAt: serverTimestamp()
        });
      ...
      const txtContatos = window.contatosExtrasTemp
        .map(c => `• ${c.nome} - ${c.telefone}`)
        .join("\n");

      const clienteSnap = await getDoc(doc(db, "clientes", docRef.id));
      const obsAtual = clienteSnap.exists() ? (clienteSnap.data().observation || "") : "";
    ...
    setTimeout(async () => {
      try {
        ...
        // 📧 EMAIL
        try {
          await emailjs.send("template_pf5wind", {
...
function startNotificacoes() {
  ...
  onSnapshot(q, snap => {
    snap.forEach(async d => {
      ...
      if (n.tipo !== "SOLICITACAO_AGENDAMENTO") {
...
if (n.tipo === "SOLICITACAO_AGENDAMENTO") {
  ...
  if (n.status === "ACEITA") {
    statusHtml = `<div style="margin-top:8px;color:#22c55e;font-weight:600;">
      ✅ Pedido aceito
    </div>`;
  }

  else if (n.status === "RECUSADA") {
    statusHtml = `<div style="margin-top:8px;color:#ef4444;font-weight:600;">
      ❌ Pedido recusado
    </div>`;
  }
    ...
    actionsHtml = `
      <div style="margin-top:8px; display:flex; gap:8px;">
        <button onclick="aceitarSolicitacaoAgendamento('${d.id}', '${n.clienteId}')">
          ✅ Aceitar
        </button>
        <button onclick="confirmarRecusa('${d.id}', '${n.clienteId}')">
          ❌ Recusar
        </button>
      </div>
    `;
...
window.confirmarRecusa = function (notificacaoId, clienteId) {
  const ok = confirm("Tem certeza que deseja recusar este agendamento?");

  if (!ok) return;

  recusarSolicitacaoAgendamento(notificacaoId, clienteId);
};
...
window.openModalObservacoes = async function (cliente) {
  try {
    ...
    document.getElementById("editAgDesc").value =
      ...
      // ✅ Tipo (reagendar)
    const tipoSalvo = clienteAtualizado.agendamento?.tipo || "TREINAMENTO";
    const selectTipo = document.getElementById("editAgTipo");
    const outroInput = document.getElementById("editAgTipoOutro");

    if (selectTipo) {
      // Se o tipo salvo não for uma opção fixa, vira OUTRO
...
window.salvarNovoAgendamento = async function () {
  ...
  const clienteNome = window.clienteAtual?.name;
  ...
  if (!clienteId) {
    alert("Cliente inválido. Abra o cliente antes de agendar.");
    return;
  }
  ...
  try {
    ...
    // ✅ cria também no Google Agenda (sem modal)
   // ✅ cria também no Google Agenda (sem modal)
try {
  const startISO = new Date(`${data}T${hora}:00-03:00`).toISOString();

  const minutos = Number(duracao || 60);
  const endDate = new Date(startISO);
  endDate.setMinutes(endDate.getMinutes() + minutos);
  const endISO = endDate.toISOString();

  const created = await createCalendarEvent({
    suporteId: session.user.uid,
    title: `AGENDAMENTO - ${clienteNome || ""}`.trim(),
  ...
  console.log("✅ Evento criado no Google:", created);

} catch (err) {
  console.warn("⚠️ Falha ao criar no Google Agenda:", err);
}
...
document.addEventListener("DOMContentLoaded", () => {
  document
    .getElementById("agendamentoOutroSuporteCheck")
    ?.addEventListener("change", window.toggleAgendamentoOutroSuporte);
});
...
window.onEditAgTipoChange = function (value) {
  const outro = document.getElementById("editAgTipoOutro");
  if (!outro) return;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (btn) btn.addEventListener("click", openAgendaModal);

  // botões do modal
  document.getElementById("agenda-prev")?.addEventListener("click", agendaPrevMonth);
  document.getElementById("agenda-next")?.addEventListener("click", agendaNextMonth);
  document.getElementById("agenda-today")?.addEventListener("click", agendaToday);
});
document.addEventListener("DOMContentLoaded", () => {
  document.getElementById("agenda-close")?.addEventListener("click", closeAgendaModal);
});

window.openQuickClientModal = function () {
  openModalById("modal-quick-client");

  document.getElementById("quickClientName").value = "";
  document.getElementById("quickClientTelefone").value = "";
};

window.confirmQuickClient = async function () {
  const nome = document.getElementById("quickClientName")?.value.trim();
  const telefone = document.getElementById("quickClientTelefone")?.value.trim();

  if (!nome || !telefone) {
    alert("Preencha Nome e Telefone do contato");
    return;
  }

  // ✅ só salva temporário na memória (não cria cliente novo)
  window.contatosExtrasTemp.push({ nome, telefone });

  closeModalById("modal-quick-client");

  mostrarNotificacao("✅ Contato extra adicionado! (vai salvar junto com o cliente)");
};

window.toggleAgendamentoOutroSuporte = async function () {
  const check = document.getElementById("agendamentoOutroSuporteCheck");
  const wrap = document.getElementById("agendamentoOutroSuporteWrap");
  const select = document.getElementById("agendamentoOutroSuporte");

  if (!check || !wrap || !select) return;

  if (check.checked) {
    wrap.style.display = "block";
    await loadSuportes("agendamentoOutroSuporte");
  } else {
...
window.aceitarSolicitacaoAgendamento = async function (notificacaoId, clienteId) {
  try {
    showLoading("Aceitando agendamento...");
    ...
    const ag = cliente.agendamento || {};

    if (!ag.data || !ag.hora) {
      mostrarNotificacao("❌ Agendamento incompleto");
      return;
    }

    const suporteSnap = await getDoc(doc(db, "users", auth.currentUser.uid));
    const suporte = suporteSnap.exists() ? suporteSnap.data() : {};

    const suporteEmail = suporte.email || null;
    const suporteNome = suporte.name || auth.currentUser.email || "Suporte";
    ...
    const startISO = ag.startISO || new Date(`${ag.data}T${ag.hora}:00`).toISOString();
    const endISO =
      ag.endISO ||
      new Date(new Date(startISO).getTime() + (Number(ag.duracaoMin || 60) * 60000)).toISOString();

    if (suporteEmail) {
      const ev = await createCalendarEvent({
        title: `${ag.tipo || "Agendamento"} - ${cliente.name || "Cliente"}`,
        description: ag.descricao || "",
        startISO,
        endISO,
        calendarId: suporteEmail,
        gerarMeet: true
      });
    ...
    await updateDoc(clienteRef, {
      ...
      agendamento: {
        ...
        aprovadoPorSuporte: true,
        aprovadoPorId: auth.currentUser.uid,
        aprovadoPorNome: suporteNome
      },
    ...
    mostrarNotificacao("✅ Agendamento aceito com sucesso");
  } catch (err) {
    console.error("Erro ao aceitar solicitação:", err);
    mostrarNotificacao("❌ Erro ao aceitar solicitação");
  } finally {
    hideLoading();
  }
};

window.recusarSolicitacaoAgendamento = async function (notificacaoId, clienteId) {
  try {
    showLoading("Recusando solicitação...");

    await updateDoc(doc(db, "clientes", clienteId), {
      status: "espera",
      "agendamento.pendenteAprovacao": false,
      "agendamento.aprovadoPorSuporte": false,
      updatedAt: serverTimestamp()
    });
    ...
    mostrarNotificacao("❌ Solicitação recusada");
  } catch (err) {
    console.error("Erro ao recusar solicitação:", err);
    mostrarNotificacao("❌ Erro ao recusar solicitação");
  } finally {
    hideLoading();
  }
};

// expõe só o que realmente existe (evita quebrar o main.js)
...
window.agendaPrevMonth = agendaPrevMonth;
window.agendaNextMonth = agendaNextMonth;
window.agendaToday = agendaToday;

// ❌ REMOVIDO porque estava quebrando o arquivo:
