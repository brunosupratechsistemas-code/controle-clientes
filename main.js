...
try {
  ...
  // 🔹 1) Salva cliente (PRECISA esperar)
...
// ✅ salva contatos extras + escreve automático nas observações
if (window.contatosExtrasTemp?.length > 0) {

  // 1) salva na subcoleção contatos
  ...
  // 3) salva automático no campo observation
...
await updateDoc(doc(db, "clientes", docRef.id), {
  ...
  // 4) limpa a lista pra não repetir no próximo cadastro
  window.contatosExtrasTemp = [];
}



  // 🔹 2) Notificação (PRECISA esperar p/ garantir registro)
  if (suporteId) {
  ...
  // ✅ 3) FECHA MODAL IMEDIATAMENTE (sem esperar WhatsApp/E-mail)
  closeModalById("modal-add-client");
  hideLoading();
  // se você tiver toast:
  // mostrarNotificacao("✅ Cliente salvo com sucesso!");
  // ou showToast("✅ Cliente salvo com sucesso!");

  // ✅ 4) BACKGROUND: E-mail + WhatsApp (não trava UI)
  setTimeout(async () => {
    try {
      if (!suporteId) return;
...
`;
      ...
      console.error("❌ Erro no background (email/whatsapp):", bgErr);
    }
  }, 0);

  // ✅ encerra aqui (não deixa cair no close/hide do final antigo)
  return;
