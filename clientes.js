...
if (data.status === "pendente_agendamento") {
  const pend = document.createElement("div");
  pend.style.fontSize = "12px";
  pend.style.marginTop = "6px";
  pend.style.fontWeight = "600";
  pend.style.color = "#fbbf24";
  pend.innerText = "⏳ Aguardando aceitação do suporte";
...
// 🔁 só sincroniza se existir agendamento
if (window.clienteAtual?.agendamento?.startISO) {
  fetch("https://supratech-whatsapp.fly.dev/sync-wa-jobs", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      clienteId,
      tipo: window.clienteAtual.agendamento.tipo,
      data: window.clienteAtual.agendamento.data,
      hora: window.clienteAtual.agendamento.hora,
      startISO: window.clienteAtual.agendamento.startISO,
...
const outroCheck = document.getElementById("agendamentoOutroSuporteCheck");
const outroWrap = document.getElementById("agendamentoOutroSuporteWrap");
const outroSelect = document.getElementById("agendamentoOutroSuporte");

if (outroCheck) outroCheck.checked = false;
if (outroWrap) outroWrap.style.display = "none";
if (outroSelect) outroSelect.value = "";
  // reset UI
  erroEl.innerText = "";
...
window.saveAgendamento = async function () {
  const agendarOutroSuporte =
  document.getElementById("agendamentoOutroSuporteCheck")?.checked || false;

const outroSuporteId =
  document.getElementById("agendamentoOutroSuporte")?.value || "";
  const target = agendamentoTarget;
if (agendarOutroSuporte && !outroSuporteId) {
  document.getElementById("agendamentoErro").innerText =
    "Selecione o suporte que vai receber a solicitação.";
  return;
}

const suporteResponsavelId = agendarOutroSuporte
  ? outroSuporteId
  : session.user.uid;
...
// ===============================
// 🚨 NOVO FLUXO: OUTRO SUPORTE
// ===============================
if (agendarOutroSuporte) {
...
await updateDoc(clienteRef, {
  ...
  agendamento: {
    ...
    criadoPorNome: session.user.name || session.user.email,

    suporteResponsavelId: outroSuporteId,

    pendenteAprovacao: true,
    aprovadoPorSuporte: false
  },
  ...
  await addDoc(collection(db, "notificacoes"), {
    ...
    clienteId,
    clienteNome: clienteNomeTarget,
    suporteId: outroSuporteId,

    solicitadoPorId: session.user.uid,
    solicitadoPorNome: session.user.name || session.user.email,
  ...
  mostrarNotificacao("📨 Solicitação enviada para o suporte");

  return; // ⛔ PARA AQUI (NÃO CRIA GOOGLE)
}
  try {
...
const tipoFmt = mapaTipos[normalizado] || tipo;
  ...
  if (tipoFmt === "APRESENTAÇÃO") {
    msg = `*${suporteExibicao}*\n\nOlá *{nome}*. Me chamo *${suporteExibicao}* (responsável técnico) e trabalho na empresa SupraTech Sistemas.
...
Foi agendado um *TREINAMENTO* referente ao sistema no dia *${dataFmt}* às *${hora} (horário de Brasília)*. Posso confirma?`;
  ...
  // ✅ Fallback caso tenha outro tipo no futuro
    ...
    msg = `Olá *{nome}*. Me chamo *${suporteExibicao}* (responsável técnico) e trabalho na empresa SupraTech Sistemas.
...
const novaMensagemCliente = montarMsgConfirmacaoAgendamento({
  clienteNome: clienteData.name,
  suporteNome,
  tipo,
  data,
  hora,
  meetLink: clienteData.googleMeetLink || ""
});

// ===============================
// 🔥 ATUALIZA JOB WHATSAPP AUTOMÁTICO SEMPRE
// ===============================
await updateDoc(doc(db, "clientes", clienteId), {
  agendamento: {
    tipo,
    ...
    waMensagemCliente: novaMensagemCliente
  },
  updatedAt: serverTimestamp()
});


// ✅ Agora sim sincroniza job automático
fetch("https://supratech-whatsapp.fly.dev/sync-wa-jobs", {
  method: "POST",
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify({
    ...
    hora,
    startISO,
    enviarParaCliente: clienteData.waAuto?.cliente !== false,
    enviarParaSuporte: clienteData.waAuto?.suporte !== false
  })
}).catch(err => {
...
// 🔁 só sincroniza se o cliente estiver agendado
await fetch("https://supratech-whatsapp.fly.dev/sync-wa-jobs", {
  method: "POST",
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify({
    clienteId: pendingWA.clienteId,
    tipo: pendingWA.tipo,
    data: pendingWA.startISO.slice(0, 10),
    hora: pendingWA.hora,
    startISO: pendingWA.startISO,
